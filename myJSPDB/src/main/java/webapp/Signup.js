function validateId() {
      var userId = document.getElementById("customer_user_id").value;
      var idPattern = /^[A-Za-z0-9]{4,12}$/; // 영어 대소문자 및 숫자, 4-12자
      if (!idPattern.test(userId)) {
          document.getElementById("idError").innerHTML = "아이디는 영어 대소문자와 숫자로 4자 이상 12자 이하로 입력해야 합니다.";
          return false;
      } else {
  ...
  function validatePassword() {
      ...
      if (!passwordPattern.test(password)) {
          document.getElementById("pwdError").innerHTML = "비밀번호는 8자 ~ 16자, 알파벳, 숫자, 특수문자를 포함해야 합니다.";
          return false;
      } else {
  ...
  function validateForm() {
      ...
      submitButton.disabled = !(isIdValid && isPasswordValid && isNameValid && isCompanyValid && isPhoneValid && isEmailValid);
  }
