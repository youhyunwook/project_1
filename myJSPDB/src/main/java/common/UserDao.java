...
public class UserDao {
	...
	// findId 아이디 찾기
	public String findId(User user) throws ClassNotFoundException {
	    ...
	    try {
	        ...
	        pstmt = connection.prepareStatement("SELECT customer_user_id FROM user WHERE customer_user_name = ? AND customer_user_email = ?");
	        pstmt.setString(1, user.getCustomer_user_name());
	        pstmt.setString(2, user.getCustomer_user_email());
	        rs = pstmt.executeQuery();
	        
	        if (rs.next()) {
	            customer_user_id = rs.getString("customer_user_id"); // 사용자 ID 저장
	        }
	    } catch (ClassNotFoundException e) {
	        e.printStackTrace();
	    } catch (SQLException e) {
	    ...
	    return customer_user_id; // 사용자 ID 또는 null 반환
	}

	
	// findId 비밀번호 찾기
	public String findPwd(User user) throws ClassNotFoundException {
	    ...
	    try {
	        ...
	        if (rs.next()) {
	        	customer_user_pwd = rs.getString("customer_user_pwd"); // 사용자 ID 저장
	        }
	    } catch (ClassNotFoundException e) {
	        e.printStackTrace();
	    } catch (SQLException e) {
